...
Use `bench/reasoning_check.py` with baseline comparison, `bench/agent_contract.py` for actual streaming/tool/image behavior, and `bench/context_capacity.py` for tokenizer-counted long inputs with randomized planted facts and synthesis. Near-500K validation uses approximately 480000 input tokens plus 8192 output reserve. Concurrent capacity checks use `--hold-output` to retain long contexts during generation. Separately observe peak cache occupancy and preemption counters; completed requests alone do not prove their full contexts coexisted.
...
Model loading reports 65.41 GiB per rank, versus the FP8 baseline's 87.83 GiB. Available KV memory is 33.53 GiB; the shared TP pool is **3913624 tokens**, counted once. This native-context allocation does not qualify the extended profile by itself. All 12 reasoning/retrieval checks passed with no baseline regressions, including the 104088-token retrieval fixture. The streamed tool round trip, red/blue image check, and red-video check passed. The completed comparisons below remain native-profile measurements.
...
The measurement window recorded 129024 generation tokens, 124749 draft tokens and 87460 accepted draft tokens: 70.1% acceptance, 2.103 accepted tokens per draft. Native MTP1 completed three clean recorded repetitions, with median aggregate throughput of 44.06, 69.44, 92.51, 113.08, 127.83, and 145.10 tok/s at concurrency 1 through 6. MTP0 completed one diagnostic sweep at 25.54, 50.22, 69.30, 86.06, 94.79, and 108.50 tok/s; further repetitions were cancelled. MTP3 won the matched MTP1 comparison and remains selected. The single MTP0 sweep is not a repeated qualification.
...
The selected 500K profile is serving with 4,304,597 shared KV tokens and passed the near-480K semantic check. DSH delivery is complete. Final-profile isolated speed, concurrent near-limit capacity, recovery/rollback drills, and the mixed-workload soak remain unqualified. The earlier 1M target was superseded by the selected 500K limit.
